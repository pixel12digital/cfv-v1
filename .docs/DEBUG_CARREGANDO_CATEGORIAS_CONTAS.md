...
|----------|-------------|
| **SyntaxError na linha 354** | 1) DevTools → **Sources** → abrir o documento da URL → ir na **linha 354** e ver o trecho exato. 2) View Source: procurar `categorias-despesa-view-v2` e `var apiUrl =`. Se aparecer `var apiUrl = <?=` ou `var apiUrl = "/caminho"` com quebra na linha seguinte → **cache** (OPcache, SW ou proxy). 3) Rodar `tools/clear-opcache.php?clear=1`, reiniciar PHP no painel, desregistrar o Service Worker (Application → Service Workers → Unregister) e recarregar. |
